...
var fromString = (function () {
    ...
    function fromString(s) {
        if (typeof s !== "string") {
            throw new TypeError("fromString(string) expected String for 'string'");
        }
...
var format = (function () {
    ...
    function format(s, date) {
        if (!isDate(date)) {
            throw new TypeError("format(format, date) expected Date for 'date'");
        }
...
function floor(intervalName, date) {
    if (!isInterval(intervalName)) {
        ...
        throw new TypeError("floor(interval, date) expected Date for 'date'");
    }
...
function ceil(intervalName, date) {
    if (!isInterval(intervalName)) {
        ...
        throw new TypeError("ceil(interval, date) expected Date for 'date'");
    }
...
function add(unitName, n, date) {
    if (!isUnit(unitName)) {
        ...
        throw new TypeError("add(unit, n, date) expected Number for 'n'");
    } else if (!isDate(date)) {
        throw new TypeError("add(unit, n, date) expected Date for 'date'");
    }
    var date2 = new Date(date.getTime()),
...
var diff = (function () {
    ...
    function diff(unitName, date1, date2) {
        if (!isUnit(unitName)) {
            ...
            throw new TypeError("diff(unit, date1, date2) received unexpected value for 'unit'");
        }
...
function range(intervalName, date1, date2, step) { // returns an array of dates >= date1 and < date2
    ...
    if (!isInterval(intervalName)) {
        ...
        throw new TypeError("range(interval, date1, date2, step) expected Dates for 'date1' and 'date2'");
    } else if (typeof step !== "number") {
        throw new TypeError("range(interval, date1, date2, step) expected Number for 'n'");
    }
