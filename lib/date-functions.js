...
function dateCeil(intervalName, date) {
    ...
    var date2 = dateFloor(intervalName, date),
        interval = intervals[intervalName] || units[intervalName],
...
// dateAdd(String, Number, Date) -> Date
...
// dateDiff(String, Date, Date) -> Number
...
var dateDiff = (function () {
    ...
    function diffTime(unitName, date1, date2) { // ms, second, minute, hour
        var ms = parts[units[unitName][0]][2],
            n = (date2 - date1) / ms;
        return Math[date1 < date2 ? "floor" : "ceil"](n);
    }
    ...
    function dateDiff(unitName, date1, date2) {
        if (!isUnit(unitName)) {
        ...
        var partsIndex = units[unitName][0],
            ...
            n = diffFunction(unitName, date1, date2);
        return n === 0 ? Math.abs(n) : n;
    }
    return dateDiff;
}());

// dateRange(String, Date, Date, Number) -> Array

function dateRange(intervalName, date1, date2, step) { // returns an array of dates >= date1 and < date2
    step = step === undefined ? 1 : step; // default
    if (!isInterval(intervalName)) {
        throw new Error("dateRange(interval, date1, date2, step) received unexpected value for 'interval'");
    } else if (!isDate(date1) || !isDate(date2)) {
        throw new TypeError("dateRange(interval, date1, date2, step) expected Dates for 'date1' and 'date2'");
    } else if (typeof step !== "number") {
        throw new TypeError("dateRange(interval, date1, date2, step) expected Number for 'n'");
    }
    step = Math.max(1, Math.round(step)); // step must be an integer >= 1
    var unitName = intervals[intervalName] ? intervals[intervalName][4] : intervalName,
        ...
        a = [],
        d = dateCeil(intervalName, date1);
    while (d < date2) {
        a.push(d);
        d = dateAdd(unitName, unitCoeff * step, d);
    }
    return a;
}
