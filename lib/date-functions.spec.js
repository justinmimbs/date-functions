import { isDate, isDateString, dateFromString, dateToUTC, dateFormat, dateFloor, dateCeil, dateAdd, dateDiff, dateRange } from './date-functions';
...
describe('dateFromString(string)', () => {
  ...
  it('parses ISO-8601 strings and returns Date objects as expected', () => {
    ...
    expect(dateFromString('2012-09-27T22:56:00.5555')).toEqual(new Date(2012, 8, 27, 22, 56, 0, 556));
    expect(dateFromString('2012-09-27T22:56:00.5')).toEqual(new Date(2012, 8, 27, 22, 56, 0, 500));
    expect(dateFromString('2012-09-27T22:56.5')).toEqual(new Date(2012, 8, 27, 22, 56, 30, 0));
    expect(dateFromString('2012-09-27T22.75')).toEqual(new Date(2012, 8, 27, 22, 45, 0, 0));
    expect(dateFromString('20120927T225600.555')).toEqual(new Date(2012, 8, 27, 22, 56, 0, 555));
    expect(dateFromString('2012-09-27T00Z')).toEqual(dateFromString('2012-09-27T00:00:00.000+00:00'));
  });
});

describe('dateToUTC(date)', () => {
  it('throws a TypeError when `date` is not a Date', () => {
    expect(() => dateToUTC('')).toThrowError(TypeError);
    expect(() => dateToUTC('', null)).toThrowError(TypeError);
    expect(() => dateToUTC('', true)).toThrowError(TypeError);
    expect(() => dateToUTC('', 0)).toThrowError(TypeError);
    expect(() => dateToUTC('', '')).toThrowError(TypeError);
    expect(() => dateToUTC('', {})).toThrowError(TypeError);
  });

  it('returns a Date representing `date` in UTC time', () => {
    const date1 = new Date();
    const date2 = dateToUTC(date1);
    const msTimezoneOffset = date1.getTimezoneOffset() * 60000;
    expect(date2 - date1).toBe(msTimezoneOffset);
  });
...
describe('dateFormat(format, date)', () => {
  ...
  it('replaces timezone-offset tokens as expected', () => {
    expect(dateFormat('O', new Date())).toMatch(/[+\-][012]\d[012345]\d/);
    expect(dateFormat('P', new Date())).toMatch(/[+\-][012]\d:[012345]\d/);
  });
...
describe('dateDiff(unit, date1, date2)', () => {
  ...
  it('does not mutate `date1` or `date2`', () => {
    const date1 = new Date();
    ...
    expect(date1.getTime()).toBe(date1Value);
    expect(date2.getTime()).toBe(date2Value);
  });
});
...
describe('dateRange(interval, date1, date2[, step])', () => {
  ...
  it('does not mutate `date1` or `date2`', () => {
    const date1 = new Date();
    const date2 = new Date(date1.getFullYear() + 1, 0, 1);
    const date1Value = date1.getTime();
    const date2Value = date2.getTime();
    dateRange('day', date1, date2);
    expect(date1.getTime()).toBe(date1Value);
    expect(date2.getTime()).toBe(date2Value);
  });
});
